...
export function AgentHeader({ title, subtitle, agentName, agentId }: AgentHeaderProps) {
  ...
  useEffect(() => {
    const fetchUser = async () => {
      ...
      if (user) {
        setUserData({
          name: agentName || user.user_metadata?.name || user.email?.split("@")[0] || "Agent",
          email: user.email || "",
        })
      }
    }
  ...
  useEffect(() => {
    const fetchNotifications = async () => {
      ...
      const [requestsRes, payoutsRes] = await Promise.all([
        supabase
          .from("agent_requests")
          .select("id, shop_id, status, requested_at, shops:shop_id (shop_name)")
          .eq("agent_id", agentId),
        supabase
          .from("payouts")
      ...
      ;(requestsRes.data || []).forEach((req: any) => {
        if (req.status === "pending") {
          ...
          notifs.push({
            ...
            message: `${req.shops?.shop_name || "A shop"} approved your partnership request`,
            date: new Date(req.requested_at).toLocaleDateString(),
            read: false,
          })
          ...
          notifs.push({
            ...
            message: `${req.shops?.shop_name || "A shop"} rejected your partnership request`,
            date: new Date(req.requested_at).toLocaleDateString(),
            read: false,
          })
        }
      ...
      ;(payoutsRes.data || []).forEach((p: any) => {
        if (p.status === "paid") {
          notifs.push({
            ...
            message: `₹${Number(p.amount_paid || 0).toLocaleString("en-IN")} has been paid to your account`,
          ...
          notifs.push({
            ...
            message: `₹${Number(p.amount_paid || 0).toLocaleString("en-IN")} is being processed`,
            date: new Date(p.created_at).toLocaleDateString(),
            read: false,
          })
        }
  ...
  const handleAcceptRequest = async (requestId: number) => {
    await supabase.from("agent_requests").update({ status: "approved" }).eq("id", requestId)
    dismissNotification(`req-${requestId}`)
  }

  const handleRejectRequest = async (requestId: number) => {
    await supabase.from("agent_requests").update({ status: "rejected" }).eq("id", requestId)
    dismissNotification(`req-${requestId}`)
  }
  ...
  const displayName = agentName || userData?.name || "Agent"
            ...
            >
              ...
              <div className="hidden lg:block text-left">
                ...
                <p className="text-xs text-slate-500">
                  Agent ID: AGT-{String(agentId || 0).padStart(4, "0")}
                </p>
              </div>
